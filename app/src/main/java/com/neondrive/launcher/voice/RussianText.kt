...
object WakeWord {
     ...
     * него не прозвучало: это нормальный сценарий, когда человек зовёт, ещё не
     * решив, что попросить. Оболочка в этом случае просто продолжает слушать —
     * молча, показывая плашку; вслух переспрашивает только сеанс, начатый
     * кнопкой (см. [com.neondrive.launcher.voice.VoiceAssistant]).
     */
    ...
    /**
     * Строгое совпадение — только то, что перечислено в грамматике.
     *
     * Используется единственным местом: решением «проснуться». Режим ожидания и
     * так не умеет выдать ничего, кроме слов грамматики, поэтому расстояние
     * Левенштейна не добавляет там распознаванию ничего, зато добавляет ложных
     * срабатываний. А цена ошибок разная: непонятое обращение человек повторит,
     * ложное пробуждение он не просил вовсе.
     */
    fun matchesStrict(word: String): Boolean =
        RussianText.normalize(word) in GRAMMAR_VARIANTS

    /** Похоже ли слово на обращение. Мягкое сравнение — для разбора уже начатой команды. */
