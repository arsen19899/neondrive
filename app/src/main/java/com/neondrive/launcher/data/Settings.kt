 ...
 * «карта во фрейме» достигается одним из двух обходных путей. Третий, «обычный»
 * режим (панель рисует свой HUD и разворачивает навигацию только по нажатию) убрали:
 * он на практике неудобен — лишний шаг перед тем как увидеть настоящую карту.
 */
enum class MapMode(val label: String, val hint: String) {
    /**
     * Настоящая карта, нарисованная самой оболочкой внутри панели.
     *
     * Единственный режим, который даёт «часть экрана — карта, часть — оболочка»
     * без всяких условий: карта рисуется в нашем же окне, поэтому ни freeform,
     * ни разрешения на оверлей, ни особенностей прошивки не требуется вообще.
     * Тайлы — OpenStreetMap через osmdroid, ключ не нужен. Маршрутную навигацию
     * с голосом по-прежнему ведёт установленный навигатор: он открывается на
     * весь экран кнопкой, когда действительно нужен.
     */
    EMBEDDED(
        "Своя карта",
        "Оболочка рисует настоящую карту прямо в панели — работает на любом " +
            "устройстве без настройки. Навигатор открывается на весь экран по кнопке"
    ),
...
data class LauncherSettings(
     ...
     *
     * FRAME требует freeform-режима прошивки, которого на бюджетных ГУ (Unisoc
     * UIS8581/SC9863, MTK AC8257 и подобные) либо нет вовсе, либо он выключен и
     * включается только через adb с перезагрузкой. OVERLAY требует разрешения
     * «Поверх других приложений» и всё равно рисует навигатор во весь экран,
     * пряча его половину под панелями. EMBEDDED не требует ничего: карту рисует
     * сама оболочка внутри своего окна, поэтому «часть экрана — карта, часть —
     * оболочка» работает на любой прошивке сразу после установки.
     */
     ...
     * От этого же зависит, с какой стороны экрана остаётся свободная полоса, когда
     * навигатор поднят «во фрейме», — см. FrameSafeArea в NeonRoot.
     */
    val mapSide: SidebarSide = SidebarSide.RIGHT,
    /**
     * Какую долю ширины экрана занимает навигация, %.
     *
     * Одно число управляет всеми тремя режимами сразу, чтобы «половина экрана»
     * означала половину везде:
     *  • на рабочем столе по нему считается ширина панели карты;
     *  • в режиме FRAME границы этой панели становятся границами плавающего окна
     *    навигатора — то есть навигатор физически занимает ровно эту долю;
     *  • в режиме OVERLAY по нему считается суммарная ширина панелей оболочки,
     *    и свободной под карту остаётся ровно эта же доля.
     *
