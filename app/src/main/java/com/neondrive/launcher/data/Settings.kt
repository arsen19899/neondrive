...
data class LauncherSettings(
     ...
     *
     * По умолчанию OVERLAY, а не FRAME. FRAME требует freeform-режима прошивки,
     * которого на большинстве бюджетных ГУ (Unisoc UIS8581/SC9863, MTK AC8257 и
     * подобные) либо нет вовсе, либо он выключен и включается только через adb.
     * На таких прошивках FRAME молча вырождался в «навигатор на весь экран без
     * панелей оболочки» — то есть заводская настройка не работала ровно там, где
     * стоит большинство установок. OVERLAY работает на любой прошивке, ему нужно
     * лишь разрешение «Поверх других приложений», которое запрашивается из UI.
     */
    val mapMode: MapMode = MapMode.OVERLAY,
    /**
